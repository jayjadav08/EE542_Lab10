...
def model_fit_predict(X_train,X_test,y_train,y_test):
	...
	tuned_parameters = {
		'KNNClassifier':{'n_neighbors':[5],'p':[5]}
    }
	scores= {}
	for key in models:
...
if __name__ == '__main__':
	...
	logging.info("Percentage of tumor cases in training set is {}".format(sum(y_train)/len(y_train)))
	logging.info("Percentage of tumor cases in test set is {}".format(sum(y_test)/len(y_test)))
